...
/**
 * @brief Returns a new instance of PrintWrapper for each log operation.
 *
 * This ensures thread safety by creating a new wrapper for each log operation.
 */
...
Traceable::PrintWrapper::PrintWrapper(Traceable &traceable, const Level level, const int line)
    : traceable(&traceable), level(level), line(line)
{
...
{
    if (traceable && traceable->isEnabled(level))
    {
...
Traceable::Traceable(const __FlashStringHelper *const functionName,
                     ...
                     Print *output)
{
    Settings** existingSettingsPtr = settingsMap.get(functionName);
    if (existingSettingsPtr == nullptr || *existingSettingsPtr == nullptr)
    {
        settings = new Traceable::Settings(functionName, compileTimeLevel, output, Traceable::Level::INFO);
        settingsMap.insert(functionName, settings);
    }
    else
    {
        settings = *existingSettingsPtr;
    }
}

/**
 * @brief Destructor that cleans up settings.
 */
Traceable::~Traceable()
{
    if (settings != nullptr)
    {
        delete settings;
        settingsMap.remove(settings->getFunctionName());
        settings = nullptr;
    }
}
...
{
    if (!isEnabled(level) || settings == nullptr)
        return;
