...
public:
     ...
     * @param output The output device to use for logging (defaults to Serial).
     */
    ...
    void setLevel(const Level level) { if (settings) settings->setLevel(level); }
    Level getLevel() const { return settings ? settings->getLevel() : Level::OFF; }
    bool isEnabled(const Level level) const { return settings && static_cast<uint8_t>(level) <= static_cast<uint8_t>(settings->getLevel()); }

    // Output redirection

    void setOutput(Print &output) { if (settings) settings->setPrinter(&output); }
    Print *getOutput() { return settings ? settings->getPrinter() : nullptr; }
