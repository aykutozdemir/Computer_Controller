...
public:
  ...
  {
#if defined(ESP32)
    PGM_P (*_getCommandPgmFn)(PGM_VOID_P) = _command->getCommandPgmFn;
#else
    PGM_P (*_getCommandPgmFn)(PGM_VOID_P) = (PGM_P (*)(PGM_VOID_P)) pgm_read_word(&(_command->getCommandPgmFn));
#endif
  ...
  {
#if defined(ESP32)
    PGM_P (*_getDescriptionPgmFn)(PGM_VOID_P) = _command->getDescriptionPgmFn;
#else
    PGM_P (*_getDescriptionPgmFn)(PGM_VOID_P) = (PGM_P (*)(PGM_VOID_P)) pgm_read_word(&(_command->getDescriptionPgmFn));
#endif
  ...
  {
#if defined(ESP32)
    const impl::ArgConstraint *(*_getArgsPgmFn)(PGM_VOID_P, uint8_t *) = _command->getArgsPgmFn;
#else
    const impl::ArgConstraint *(*_getArgsPgmFn)(PGM_VOID_P, uint8_t *) = (const impl::ArgConstraint *(*)(PGM_VOID_P, uint8_t *)) pgm_read_word(&(_command->getArgsPgmFn));
#endif
  ...
  {
#if defined(ESP32)
    void (*function)(SerialCommands &, Args &) = _command->function;
#else
    void (*function)(SerialCommands &, Args &) = (void (*)(SerialCommands &, Args &)) pgm_read_word(&(_command->function));
#endif
    if (function)
    {
  ...
  {
#if defined(ESP32)
    *commands = (const Command *)(_command->subcommands);
    *count = _command->subcommandsCount;
#else
    *commands = (const Command *)pgm_read_word(&(_command->subcommands));
